...
fn benchmark_forward_f32(c: &mut Criterion) {
    ...
    for n in LENGTHS.iter() {
        ...
        let mut plan =
            C2CPlan32::aligned(&[len], Sign::Forward, Flag::DESTROYINPUT | Flag::ESTIMATE).unwrap();
...
fn benchmark_forward_f64(c: &mut Criterion) {
    ...
    for n in LENGTHS.iter() {
        ...
        let id = "FFTW Estimate";
        let mut plan =
            C2CPlan64::aligned(&[len], Sign::Forward, Flag::DESTROYINPUT | Flag::ESTIMATE).unwrap();
