...
mod common;
use common::{groups, ids, split_complex, sweep_complex, PLANNER_MODE_LENGTHS};

// Compares `PlannerMode::Heuristic` vs. `PlannerMode::Tune` on the same
// PhastFT FFT call. The two modes currently produce identical planners (the
// `_mode: PlannerMode` argument is ignored by `PlannerDit*::with_mode`), so
// the bench primarily pins the API surface — once `Tune` ships a real
// search, this becomes the apples-to-apples comparison.

macro_rules! planner_mode_bench {
    ($name:ident, $float:ty, $planner:ty, $fft:ident, $group:expr) => {
        fn $name(c: &mut Criterion) {
            sweep_complex::<$float, _>(c, $group, PLANNER_MODE_LENGTHS, |g, len| {
                ...
                for (id, mode) in [
                    ...
                    g.bench_function(BenchmarkId::new(id, len), |b| {
                        b.iter_batched(
                            ...
                            |(mut reals, mut imags)| {
                                $fft(&mut reals, &mut imags, Direction::Forward, &planner, &opts);
                            },
